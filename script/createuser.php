...
if ($argc == 4) {
    ...
    $a = \model\simple\Ssh::execute("root", \config\Conf::$rootpassword, escapeshellcmd('su -l ' . $login . ' -c "dtach -n /tmp/rtorrent' . $login . '.dtach rtorrent -n -o import=/home/' . $login . '/.rtorrent.rc"'));
    if ($a["error"] !== "") {
        throw new Exception("Erreur : " . $a["error"]);
    }
    $a = \model\simple\Ssh::execute("root", \config\Conf::$rootpassword, escapeshellcmd('su -l ' . $login . ' -c "dtach -n /tmp/rtorrent' . $login . '.dtach rtorrent -n -o import=/home/' . $login . '/.rtorrent.rc"'));
